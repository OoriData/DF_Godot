...
> [!IMPORTANT]
> **Structural reference** for UI element inventory — scene files, owning scripts, node trees, and the layer map. Use it to locate *where* a UI element lives.
> **Verify before trusting:** entries are point-in-time; `file:line` and notes may be stale — confirm against current code.
> **Live issues, visual-polish items, and in-flight work are NOT here** — they live in [docs/TODO.md](../TODO.md). This doc is the map, not the tracker. Update the sections below after any UI *scene/script* structural change.
...
## Known Issues, Polish & In-Flight Work

> Relocated to [docs/TODO.md](../TODO.md) on 2026-06-25. Volatile status (bugs, visual-polish items, cross-cutting tech debt, migration progress, audit-status tables) rots when interleaved with structural reference, and an agent reading this file will act on stale claims as fact.

This document is now the **structural map only**. For current work:

- **Bugs, improvements, and the sprint plan** → [TODO.md → Action Plan](../TODO.md)
- **Cross-cutting tech debt + visual-polish backlog (former P1–P9 / Cross-Cutting Summary)** → [TODO.md → Tech-Debt & Polish Backlog](../TODO.md)
- **Per-element scene/script/tree details** → the sections **above** (keep these current after structural changes).

When you change a scene tree, rename a node, or move a script, update the matching section above. When you find a *bug* or polish item, log it in TODO.md — not here.
