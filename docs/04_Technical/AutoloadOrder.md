...
> `tools/docs_check.py` fails the build on **membership, row numbering, and load order**, so this page
> cannot silently fall behind the code the way a hand-maintained list does.
>
> **What is still NOT checked: the `Lines` and `Owns` columns.** Those are hand-maintained and can drift
> without failing CI — treat a line count as a hint, not a fact, and re-count before relying on one.

> [!NOTE]
> **Verified against source 2026-07-31.** Every "Owns" cell was written from the script's actual public
> API, not from its name. Line counts are a deliberate signal: several of these are thin passthroughs,
> and prior docs described them as if they held substantial logic. See
> [DocumentationAudit § F11](../DocumentationAudit.md#f11--autoload-coverage-is-inverse-to-importance).
>
> **Three real defects were found and fixed on 2026-07-31**, all of which had survived the previous
> verification because the CI check only tested membership:
> 1. **Six entries were in the wrong load order** (bands 4–5). `SteamManager`, `AutoSellService`,
>    `PushNotificationManager`, and `GoogleAuthService` actually load *before* `DeviceStateManager` and
>    `UITheme`, not after. The order check added the same day now enforces this.
> 2. **Three line counts were stale** (`ErrorTranslator`, `SettingsManager`, `APICalls`).
> 3. **Load-order rule 4 named a signal that does not exist** (`ui_scale_changed`).
...
| 3 | `ErrorTranslator` | `Scripts/System/error_translator.gd` | 170 | **Raw API error → player-facing text.** `translate()`, plus routing predicates `is_inline_error()` and `is_premium_required()`. | [ErrorSystem](ErrorSystem.md) |
| 4 | `SettingsManager` | `Scripts/System/settings_manager.gd` | 127 | **Config persistence.** `get_value` / `set_and_save` / `save_settings` / `load_settings`. Owns the `display.fullscreen` contract — window mode changes route through here, never `DisplayServer`. | [UserSettings](UserSettings.md) |
...
| 6 | `APICalls` | `Scripts/System/api_calls.gd` | **3211** | **All HTTP.** Request queue + parallel GET pool, watchdogs, auth header injection, session token, per-endpoint request/response signals. The largest single file in the project. | [NetworkLayer](NetworkLayer.md) · [API_Reference](API_Reference.md) |
...
| # | Autoload | Script | Lines | Owns | Doc |
|---|---|---|---|---|---|
| 22 | `SteamManager` | `Scripts/System/steam_manager.gd` | 92 | `is_steam_running()`, `get_steam_id()`, `get_steam_username()`. Gates the desktop Steam login button **and** the DF+ purchase flow — `main_screen._try_show_premium_upgrade()` returns `false` without it. | [Identity](Identity.md) · [Deployment](Deployment.md) |
| 23 | `AutoSellService` | `…/Services/auto_sell_service.gd` | 362 | **Post-journey cargo diffing.** Snapshot at `user://cargo_snapshot.json`, delivery detection, receipt modal payload. `simulate_autosell()` for testing. | [AutoSellSystem](../03_Systems/AutoSellSystem.md) |
| 24 | `PushNotificationManager` | `…/Services/push_notification_manager.gd` | 142 | Per-platform token setup (`_setup_ios` / `_setup_android`) and token registration on `user_changed`. | [PushNotifications](PushNotifications.md) |
| 25 | `GoogleAuthService` | `…/Services/google_auth_service.gd` | 125 | `is_available()`, `sign_in()`, `silent_sign_in()`, `connect_account()`, `sign_out()`. | [MultiProviderAuth](MultiProviderAuth.md) |

### 6 · Device & theme

> Loaded **last**, after every consumer. Both are pure lookups with no `_ready()` dependencies, which is
> why the position is safe — but note that a script reading `UITheme` constants at *parse* time would
> still be fine, while one reading them in an earlier autoload's `_ready()` would not.

| # | Autoload | Script | Lines | Owns | Doc |
|---|---|---|---|---|---|
| 26 | `DeviceStateManager` | `Scripts/System/device_state_manager.gd` | 60 | **Orientation/platform queries only** — `get_layout_mode()`, `get_is_portrait()`. Has **no** font-scaling role. | [DeviceState](../02_UI_UX/DeviceState.md) |
| 27 | `UITheme` | `Scripts/System/ui_theme.gd` | 126 | **The authoritative colour + spacing tokens** (`METAL_*`, `SURFACE_WARM`, `TEXT_*`, `SPACE_*`). Rationale lives in DesignSystem; the constants live here. | [DesignSystem](../02_UI_UX/DesignSystem.md) |
...
4. **`ui_scale_manager` before anything that reads safe-area margins.** Its `content_scale_factor` can
   ...
   **`scale_changed(new_scale)`**, not latch the boot value. This caused the blank-screen bug (S12-7).
   *(Corrected 2026-07-31: this rule previously named a signal `ui_scale_changed`, which does not exist —
   `UI_scale_manager.gd:39` declares `scale_changed`. Connecting to the wrong name fails silently at
   runtime, which is the exact failure mode this rule exists to prevent.)*
