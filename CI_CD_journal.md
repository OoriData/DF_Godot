...
## 2026-04-22 - Fix: iOS CI Plugin Architecture Mismatch
- **`.github/workflows/_build-ios-appstore.yml`**:
  - **Root cause**: Earlier changes to `export_presets.cfg` (documented in "Local Debug vs CI Codesigning") changed `application/export_method_release=0` (Development) so developers could test locally. However, this caused the Godot CI headless export to assume it was building a Development/Debug build. Consequently, Godot copied the debug APN plugin (`apn.debug.xcframework`) instead of the release one (`apn.xcframework`). Since the main engine was compiled for Release, linking failed with `Undefined symbols for architecture arm64` because the debug plugin was compiled against debug Godot headers.
  - **Fix**: Added `"application/export_method_release": "2"` (App Store) to the dynamic python override script in the CI pipeline. This forces Godot to use the correct App Store export method during CI, which correctly triggers the release APN plugin to be linked, while allowing local developers to keep `export_method_release=0` for their local deployments.
