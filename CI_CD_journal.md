...
- **`.github/workflows/_build-ios-appstore.yml` and `ios/plugins/apn.gdip`**:
  - **Root cause 1**: Earlier changes to `export_presets.cfg` changed `application/export_method_release=0` (Development) so developers could test locally. This caused Godot to assume it was building a Debug build during the headless export.
