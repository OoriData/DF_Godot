...
const AGGREGATOR_PATH := "res://Scripts/Menus/VendorPanel/cargo_aggregator.gd"
...
func test_sell_mode_includes_bulk_fuel_when_vendor_has_positive_fuel_price():
	...
	var buckets = _aggregator().build_convoy_buckets(convoy, vendor, "sell", false, Callable(), false)
...
func test_vendor_buckets_exclude_bulk_water_when_water_price_missing_or_zero():
	...
	var buckets_missing = _aggregator().build_vendor_buckets(vendor_data_missing, false, Callable())
	...
	var buckets_zero = _aggregator().build_vendor_buckets(vendor_data_zero, false, Callable())
...
func test_sell_mode_omits_intrinsic_container_when_part_id_matches() -> void:
	var agg = _aggregator()
	var convoy_data: Dictionary = {
		"convoy_id": "cv1",
		"all_cargo": [
			{"cargo_id": "c1", "name": "Fuel Tank", "part_id": "7d1c1f56-e215-4b16-89f9-dca416a2d58e", "intrinsic_part_id": null, "fuel": 10.0, "quantity": 1},
			{"cargo_id": "c2", "name": "Water Jerry Cans", "water": 100.0, "quantity": 5}
		]
	}
	var vendor_data: Dictionary = {"vendor_id": "v1", "water_price": 10.0}
	var buckets = agg.build_convoy_buckets(convoy_data, vendor_data, "sell", false, Callable(), false)
	assert_false((buckets.get("other", {}) as Dictionary).has("Fuel Tank"), "Fuel Tank should be omitted when part_id matches intrinsic container UUID")
...
func test_vendor_buckets_omit_intrinsic_resource_containers():
	var vendor := {
		"vendor_id": "v1",
		"fuel_price": 20,
		"cargo_inventory": [
			{"cargo_id": "c1", "name": "Fuel Tank", "intrinsic_part_id": "7d1c1f56-e215-4b16-89f9-dca416a2d58e", "fuel": 10.0, "quantity": 1}
		],
		"vehicle_inventory": []
	}
	var buckets = _aggregator().build_vendor_buckets(vendor, false, Callable())
	assert_eq(int((buckets.get("resources", {}) as Dictionary).size()), 0, "Intrinsic resource containers should not appear in vendor Resources")
	assert_eq(int((buckets.get("other", {}) as Dictionary).size()), 0, "Intrinsic resource containers should not appear in vendor Other")


func test_convoy_buckets_omit_intrinsic_resource_containers_in_all_cargo():
	var vendor := {"fuel_price": 20, "cargo_inventory": [], "vehicle_inventory": []}
	var convoy := {
		"convoy_id": "c1",
		"all_cargo": [
	...
	var buckets = _aggregator().build_convoy_buckets(convoy, vendor, "sell", false, Callable(), false)
	assert_eq(int((buckets.get("resources", {}) as Dictionary).size()), 0, "Intrinsic resource containers should not appear in convoy Resources")
	assert_eq(int((buckets.get("other", {}) as Dictionary).size()), 0, "Intrinsic resource containers should not appear in convoy Other")


func test_convoy_buckets_omit_intrinsic_resource_containers_even_when_empty():
	var vendor := {"cargo_inventory": [], "vehicle_inventory": []}
	var convoy := {
		"convoy_id": "c1",
		"all_cargo": [
			{"cargo_id": "c1", "name": "Fuel Tank", "intrinsic_part_id": "7d1c1f56-e215-4b16-89f9-dca416a2d58e", "fuel": null, "water": null, "food": null, "quantity": 1}
		]
	}
	var buckets = _aggregator().build_convoy_buckets(convoy, vendor, "sell", false, Callable(), false)
	assert_eq(int((buckets.get("resources", {}) as Dictionary).size()), 0)
	assert_eq(int((buckets.get("other", {}) as Dictionary).size()), 0)
